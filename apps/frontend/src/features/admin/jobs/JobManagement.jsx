  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto text-slate-100 font-sans">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-5">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">Campus Recruitment & Job Drives</h1>
          <p className="text-xs text-slate-400 mt-1">
            Active placement drives, company postings, and candidate eligibility criteria
          </p>
        </div>

        <button className="px-3.5 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-xs font-bold text-slate-950 flex items-center space-x-1.5 shadow-lg shadow-emerald-950/40 transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {jobs.map((job) => (
          <div
            key={job._id || job.id}
            className="p-5 rounded-xl bg-[#111827] border border-slate-800 shadow-lg space-y-4 hover:border-slate-700 transition-colors"
          >
            <div className="flex items-start justify-between">
              <div>
                <h3 className="font-bold text-base text-white">{job.title}</h3>
                <p className="text-xs text-emerald-400 font-semibold mt-0.5">{job.company}</p>
              </div>
